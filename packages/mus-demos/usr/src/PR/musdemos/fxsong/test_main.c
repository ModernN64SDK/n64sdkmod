...
/*************************************************************
  ...
  FXSONG demo main source file. This demo illustrates how the
  library can allow song files to change the audio library FX
  type.

  Once executed use button A on controller one to toggle the
  song change FX flag.
   ...
   for (i=0; i<2; i++)
   {
      if (controller_data[0].button&controls[i])
      {
         if (!flags[i])
         {
            ...
            switch(i)
            {
               case 0: /* A - toggle audio FX change flag */
                  if (onoff==MUSBOOL_ON)
                     onoff=MUSBOOL_OFF;
                  else
                     onoff=MUSBOOL_ON;
