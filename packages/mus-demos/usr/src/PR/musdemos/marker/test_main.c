...
/*************************************************************
  ...
  MARKER demo main source file. This demo illustrates how to
  start songs from markers defined in the song file and how to
  attach a marker callback function.

  Once executed use button A to start the song from a marker
  (the marker the song will start from is incremented). Use
  button B to stop the song.

  When markers are encountered by the music library a callback
  function will be called which will change the backgound
  colour of the screen. The screen will changed according to
  the marker number passed back to the callback function.
  ...
  Restarting the song will reset the background colour to
  black.
...
/* number of markers in song file (valid markers = 0 to NUM_MARKERS-1) */
...
void MarkerCallback(unsigned long handle, int number);
...
void MainLoop(void)
{
   ...
   /* set marker callback function */
   MusSetMarkerCallback(MarkerCallback);
   bgindex = -1;
   bgcols[0]=GPACK_RGBA5551(0x40, 0x40, 0x40,1);
   bgcols[1]=GPACK_RGBA5551(0x80, 0x00, 0x00,1);
   bgcols[2]=GPACK_RGBA5551(0x00, 0x80, 0x00,1);
   bgcols[3]=GPACK_RGBA5551(0x00, 0x00, 0xc0,1);
   ...
   for (i=0; i<2; i++)
   {
      if (controller_data[0].button&controls[i])
      {
         if (!flags[i])
         {
            ...
            switch(i)
            {
               case 0: /* A - start from next marker */
                  bgindex=-1;
                  if (current_handle)
                  {
                     MusHandleStop(current_handle, 0);
                     while (MusHandleAsk(current_handle));
                  }
                  current_handle = MusStartSongFromMarker(tune_buf1, current_marker);
                  current_marker = (current_marker+1)%NUM_MARKERS;
                  break;
               case 1: /* B - stop song */
                  if (current_handle)
                  {
