...
void GameThread(void *ignored)
{
   ...
   /* initialize NN scheduler using "osTvType" */
   if (osTvType==0)
   ...
   nnScCreateScheduler(&sc, mode, 1);

   info[0].mess.type    = NN_SC_DONE_MSG;
   info[0].screen       = cfb_A;
   info[1].mess.type    = NN_SC_DONE_MSG;
   info[1].screen       = cfb_B;
    
   osCreateMesgQueue(&gfx_queue, gfx_messages, MAX_MESGS);
   nnScAddClient(&sc, &client, &gfx_queue);
...
void MainLoop(void)
{
   ...
   while (1)
   {
      ...
      if (drawing_flag)
      {
         ...
         } while (end_msg->type!=NN_SC_DONE_MSG);
      }
      drawing_flag = 1;
      ...
      osSendMesg(nnScGetGfxMQ(&sc), (OSMesg) task, OS_MESG_BLOCK);
   }
}
