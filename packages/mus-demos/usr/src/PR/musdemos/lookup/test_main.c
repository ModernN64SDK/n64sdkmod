...
/*************************************************************
  ...
  Once executed use buttons A and B on controller one to change
  and reset the lookup table. Button A will mix up the lookup
  table. Button B will reset the lookup table.
...
musHandle last_handle;
int lookup_count = 0;
unsigned short lookup_buffer[MAX_LOOKUP];
...
void MainLoop(void)
{
   ...
   last_handle = MusStartSong(tune_buf1);
...
void MixUpSampleBank(void)
{
   int i, count, index1, index2;
   unsigned short swap, *lookup;

   if (!lookup_count)
   {
      lookup_count = MusHandleWaveCount(last_handle);
      lookup = MusHandleWaveAddress(last_handle);
      for (i=0; i<lookup_count; i++)
        lookup_buffer[i] = lookup[i];
   }
   else
   {
      lookup = MusHandleWaveAddress(last_handle);
   }
   count = Randomish(20)+20;
   for (i=0; i<count; i++)
   {
      index1 = Randomish(lookup_count);
      index2 = Randomish(lookup_count);
      if (index1!=index2)
      {
         swap = lookup[index1];
         lookup[index1]=lookup[index2];
         lookup[index2]=swap;
      }
   }
}

void RestoreSampleBank(void)
{
   ...
   for (i=0; i<2; i++)
   {
      if (controller_data[0].button&controls[i])
      {
         if (!flags[i])
         {
            ...
            switch(i)
            {
               case 0: /* A - mix up sample lookup buffer */
                  MixUpSampleBank();
                  break;
               case 1: /* B - reset sample lookup buffer */
                  RestoreSampleBank();
                  break;
            }
         }
