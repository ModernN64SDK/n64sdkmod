...
int main(int argc, char **argv)
{
  ...
  startlba = myStr2Long(argv[2]);
  bytesize = _getSizeOfFile(argv[3])-4096;
  lbasize  = byte2LBA( disktype ,startlba ,bytesize );

  fp = myOpen("fat","w");
  startlba = rev32(startlba);
  ret = fwrite( &startlba , sizeof(u8) , sizeof(s32) , fp );
  if( ret != (s32)sizeof(s32) ) myErrStop("write failed");
